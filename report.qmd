...
This is not merely a notational coincidence. It means that every property of the Jeffreys divergence --- non-negativity, symmetry, zero if and only if $P = Q$ --- automatically applies to IV and PSI, grounding decades of ad hoc thresholds (0.02, 0.10, 0.30) in a well-understood statistical quantity [@kullback1951information].
...
@fig-iv-ci shows this applied to 9 features from the Home Credit dataset. The error bars reveal that some features with similar point estimates have very different precision --- a distinction invisible without standard errors.

::: {#fig-iv-ci}
![](images/iv_bar_chart.png){fig-alt="Horizontal bar chart showing Information Value for 9 features with human-readable names, with 95% confidence interval error bars. Features are colored by group: blue for Credit Bureau, pink for Financial/Behaviour, and purple for Categorical/Profile. Vertical dashed lines mark the conventional Weak (0.02), Medium (0.1), and Strong (0.3) IV thresholds."}

Information Value with 95% confidence intervals for 9 features from the Home Credit dataset, computed using the delta-method standard error from @eq-se-iv. Colours indicate feature group: Credit Bureau (blue), Financial/Behaviour (pink), Categorical/Profile (purple).
:::

The standard error also enables a formal hypothesis test for feature relevance. Under $H_0\!: \text{IV} = 0$ (no predictive power), the test statistic $Z = \text{IV} / \text{SE}(\text{IV})$ is asymptotically standard normal. In the Home Credit data, all 9 features reject the null at the 5\% level --- but this will not always be the case, particularly for smaller portfolios or finer binning schemes where sampling noise dominates.

### 2. Testing whether PSI drift is real

The same SE formula applies to PSI. @fig-psi shows PSI computed weekly for the number of credit inquiries against a ten-week reference period. Two phenomena are visible. Around April 2020, a sharp spike crosses the minor-shift threshold --- but the wide confidence band signals that this is driven by a collapse in loan origination volumes during COVID lockdowns (as few as 82 applications in one week), not a genuine distributional shift. More telling is the sustained elevation from May onward, where PSI hovers in the 0.03--0.07 range with the confidence band consistently above zero, suggesting a real change in inquiry behaviour post-lockdown. Without CIs, both patterns look like drift; with them, the practitioner can distinguish noise from signal.
...
PSI over time for number of credit inquiries, with 95% confidence bands from the delta method. The April 2020 spike reflects low origination volumes, not real drift; the sustained post-lockdown elevation is genuine.
:::
...
Performance--fairness trade-off for 9 features. Horizontal axis: IV against the default target (predictive power). Vertical axis: IV against applicant sex (demographic bias). Features above the dashed line ($\text{IV}_{\text{fair}} > 0.05$) fail the fairness constraint at the 95% confidence level.
:::
...
About the authors
: **Agus Sudjianto** is SVP of Risk & Technology at H2O.ai and Executive in Residence at the Center for Trustworthy AI Through Model Risk Management, University of North Carolina Charlotte. He was formerly Head of Corporate Model Risk at Wells Fargo.

: **Denis Burakov** is a Data Science Team Lead at Renmoney and Fellow of the Royal Statistical Society. He focuses on applied credit risk modelling, information-theoretic methods, and open-source tooling for responsible AI.
:::
::: {.g-col-12 .g-col-md-6}
